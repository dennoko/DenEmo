...
Drag a GameObject from the Hierarchy or use the object picker to set the primary mesh. Click the clear button (X) to clear it.

### Additional Meshes

Some avatars split their face across multiple meshes (e.g., separate eyebrow or teeth meshes). Drag each extra SkinnedMeshRenderer into a `+` slot below the primary field. Each additional mesh can be removed with its own delete button. All registered meshes share one combined shape key list.
...
**Keyword**
Type to filter by name. Multiple words separated by spaces are treated as AND conditions. Press X to clear.

> Example: Type `mouth open` to show only shape keys whose names contain both words.

**Fav**
Shows only shape keys you have starred. Click the star button on any row to toggle the favorite flag.
...
**Vertex Filter**
Press this button, then click any vertex in the SceneView. The list narrows to show only shape keys that actually move that vertex. An active filter shows the selected vertex index. Press X to clear.
...
| `|Prev` | Jump to the previous keyframe |
| `<` | Step one frame backward |
| Play / Stop | Start or stop playback (loops continuously) |
| `>` | Step one frame forward |
| `Next>|` | Jump to the next keyframe |
...
**Speed**
Playback speed multiplier (0.1x to 4x). Affects preview only; it does not change the clip.
...
- **Keyframe button** - adds or updates a keyframe at the current playhead time, using the shape's current slider value.
- **Delete button** - deletes the entire track (all keyframes) for that shape key after confirmation.
...
Below all tracks, each frame that has any keyframe shows a delete button. Pressing it deletes all keyframes across all tracks at that frame - removing a complete pose from one point in time.
...
**Keyframe button**
Appears at the far right of each row. Active icon (filled) means a keyframe exists at the current playhead time. Inactive icon (outline) means no keyframe exists at this time. Clicking the icon toggles it: stamps a keyframe at the current time; removes it.

> Example: You want the mouth to be fully closed at frame 0 without using REC. Move to frame 0, set the mouth slider to 0, click the keyframe button to stamp. Done.
...
## FX Mode (Apply to Avatar)

FX Mode is for replacing expressions inside a VRChat avatar's FX playable layer (Animator Controller). It allows you to scan existing gesture/expression animations, map them to new custom animations you created, and apply the replacements in bulk.

### AVATAR / FX LAYER

This section configures the target avatar and the Animator Controller to modify.

* **Target Avatar**:
  Drag a GameObject representing the VRChat avatar from the Hierarchy or select it using the object picker. If the GameObject contains a `VRCAvatarDescriptor`, it is used to automatically find and load the FX layer.
  *Note: Initial value is automatically populated by climbing up the Hierarchy from the active target mesh.*
* **FX Layer**:
  Drag an `AnimatorController` asset from the Project view.
  *Note: Initial value is automatically detected from the Target Avatar's FX playable layer. If no avatar or descriptor is found, it can be assigned manually.*
* **Rescan**:
  Resets both fields to their auto-detected defaults based on the currently selected Target Mesh.
* **Status message**:
  Displays the number of expression animations scanned within the loaded controller.

---

### REPLACE EXPRESSIONS List

Lists all animations used in the FX layer that animate the target mesh's blendshapes.

* **Search / Filter**:
  Type in the search field to filter by animation name. Click **Assigned only** to show only animations that have replacements mapped.
* **Expression Row**:
  * **Fold**: Appears for clips that are referenced multiple times in the controller. Expand it to select which specific usage states to replace.
  * **Animation Name**: Displays the name of the original animation in the FX layer. Shows a warning if the animation's target mesh path does not match the active Target Mesh.
  * **Replacement Slot**: Click to open a clip picker popup or drag-and-drop a new `.anim` asset directly here. Hovering over a assigned slot previews the new animation in the SceneView.
  * **Clear (X)**: Click to remove the mapped replacement.
...
* **Apply Mode**:
  * **Duplicate Mode (Recommended)**: Creates a copy of the FX controller, modifies only the copy, and assigns it to the avatar's FX layer. The original controller asset is untouched.
  * **Direct Mode**: Modifies the original FX controller asset directly (supports Undo).
* **Apply Button**:
  Click to apply the mapped replacements. A confirmation dialog will list all the replacements before execution.
...
A. Yes. Detach the timeline with Detach, minimize or move that window out of the way, and use only the keyframe buttons in the shape key list combined with REC mode. The timeline still records keyframes correctly in the background.
