...
FXモードは、VRChatアバターのFXレイヤー（Animator Controller）内の表情アニメーションを一括で差し替えるためのモードです。コントローラー内をスキャンし、既存のキーフレームアニメーションを新しく作成したカスタムアニメーションにマッピングして適用できます。

### アバター / FXレイヤー設定

編集対象のアバターとAnimator Controllerを設定します。

* **対象アバター**:
  ヒエラルキーからアバターのGameObjectをドラッグ＆ドロップするか、オブジェクトピッカーで指定します。アバターに `VRCAvatarDescriptor` が含まれている場合、アバターのFXレイヤーが自動検出されます。
  ※初期値は、選択中のターゲットメッシュの親階層から自動的に検出されて設定されます。
* **FXレイヤー**:
  Projectビューから `AnimatorController` アセットをドラッグ＆ドロップして指定します。
  ※初期値は、対象アバターのFXプレイアブルレイヤーから自動的に検出されます。アバターが検出されない場合や、FXレイヤーが未設定の場合は手動でドラッグ＆ドロップして指定できます。
* **再スキャン**:
  現在選択されているターゲットメッシュに基づいて、アバターとFXレイヤーを自動で再検出・リセットします。
* **ステータス表示**:
  読み込まれたコントローラー内から検出された、表情アニメーションの件数を表示します。

---

### 表情の差し替えリスト

FXレイヤー内で使われている、ターゲットメッシュのブレンドシェイプを動かすアニメーションの一覧です。

* **検索・絞り込み**:
  検索フィールドで名前による絞り込みが可能です。「有効のみ」（割り当て済み）チップをONにすると、差し替え先が設定されている項目のみを表示します。
* **アニメーション行**:
  * **展開トグル**: コントローラー内で同じクリップが複数箇所で使われている場合に表示されます。展開して、差し替えを適用する箇所を個別に選択できます。
  * **アニメーション名**: 差し替え元のアニメーション名です。アニメーションの対象メッシュパスが、現在のターゲットメッシュと一致しない場合は警告アイコンが表示されます。
  * **差し替えスロット**: クリックしてクリップピッカーポップアップを開くか、Projectビューから新しい `.anim` アセットを直接ドラッグ＆ドロップして割り当てます。割り当て済みのスロットにマウスホバーすると、SceneView 上で新しいアニメーションがプレビュー再生されます。
  * **解除 (✕)**: 割り当てた差し替え設定を解除します。
...
* **適用モード**:
  * **複製モード（推奨）**: FXコントローラーを複製し、複製側のみを書き換えてアバターのFXレイヤーに再設定します。元のアセットは書き換えられません。
  * **直接モード**: 元のFXコントローラーアセットを直接書き換えます（Undo対応）。
* **適用ボタン**:
  クリックすると差し替えを実行します。実行前に差し替え対象の一覧と確認ダイアログが表示されます。
